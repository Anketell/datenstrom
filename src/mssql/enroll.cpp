...
template<> impl * factory_helper< mssql::connection >::construct( const connect_params_t & params )
{
   ...
   auto timeout_str = params[ "timeout" ];
   ...
   if ( !timeout_str.empty() )
      timeout = atoi( timeout_str.c_str() );

   impl * db;

   if ( !username.empty() )
      db = new mssql::connection( username, password, server, path, variant, port, timeout );
   else
      db = new mssql::connection( server, path, variant, port, timeout );
